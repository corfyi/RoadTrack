...
import  re
import numpy as np
from tracker.homo_track.tracker import HomoTracker
from detect.detect import detect_by_speed
...
def eval(sample_name, filter_data, road_map_info,root_dir, eval_positive=False):
    ...
    homo_file = os.path.join(homo_dir,f'{seq_name}.npy')

    H = np.load(homo_file,allow_pickle=True)
    pixel_ratio,x0 = road_map_info[seq_name]
    ...
    tracker = HomoTracker(H,fps=10,match_thresh=100,iou_thresh=0.99,x0=x0)        
    anormal_tracks,anormal_frames = detect_by_speed(tracker,max_frame_id,dets_by_frame,pixel_ratio)
...
def main():
    ...
    road_map_info = {"K204": [9.2,383],"K211": [12.6,364],"K221": [9.9,399],"K249": [6.4,322],"K287": [9.7,199],"K295": [12.3,443]}

    TP, TN, FP, latency_sum = 0, 0, 0, 0

    for root_dir, eval_positive in [('datasets/P/', True), ('datasets/N/', False)]:
        for sample_name in sorted(os.listdir(root_dir)):
            tp1, tn1, fp1, latency = eval(sample_name,filter_data, road_map_info, root_dir, eval_positive)
            TP += tp1
            TN += tn1
            FP += fp1
